    ...
    <div className="space-y-6" data-testid="escrow-tab-root">
        ...
        {escrows.length === 0 ? (
          <div className="p-8 rounded-2xl glass-card text-center space-y-4" data-testid="escrow-empty-state">
            ...
            <div className="space-y-1">
              <p className="text-xs font-bold text-slate-200">No active escrows yet</p>
                    ...
                    <div className="space-y-2.5">
                      {escrow.milestones?.map((m, idx) => {
                        ...
                        const getMilestoneStatus = () => {
                          if (isCompleted) return { text: "Released", colorClass: "text-teal-400 bg-teal-500/10 border-teal-500/20" };
                          if (isDisputed) return { text: "Disputed", colorClass: "text-red-400 bg-red-500/10 border-red-500/20" };
                          if (isSubmitted) return { text: "Submitted", colorClass: "text-amber-400 bg-amber-500/10 border-amber-500/20" };
                          return { text: "Open", colorClass: "text-slate-400 bg-space-800 border-space-700/50" };
                        };
                        ...
                        return (
                          ...
                          >
                            ...
                            {!isCompleted && (
                              <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-space-850">
      ...
      {!isConnected && (
        <div className="p-8 rounded-2xl glass-card text-center space-y-4">
          ...
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-200">Connect Wallet</p>
            <p className="text-[10px] text-slate-400 max-w-[200px] mx-auto leading-relaxed">
          ...
          <button
            onClick={connect}
            type="button"
            className="w-full h-10 rounded-xl bg-gradient-to-r from-teal-400 to-primary-indigo text-xs font-bold text-white flex items-center justify-center gap-1 cursor-pointer focus-ring"
          >
            Connect Wallet
          </button>
        </div>
